...
export function initGame(mode) {
  ...
  const jumpVelocityThreshold = 0.5; // keep for corner fix
  const accelerationFactor = 0.1; // Acceleration coefficient
  const decelerationFactor = 0.15; // Deceleration coefficient

  // --- Colors ---
  const colors = {
    backgroundStart: '#e0e6eb',
    backgroundEnd: '#b0b8c1',
    hillColorFar: 'rgba(130, 140, 150, 0.3)',
    hillColorNear: 'rgba(120, 130, 140, 0.4)',
    platformBase: '#f5d7a4',
    platformEdge: '#e4a76a',
    playerBody: '#2c3e50',
    player1Headband: '#3498db',
    player2Headband: '#e74c3c',
    eyeWhite: '#ffffff',
    eyePupil: '#000000',
    indicator: '#f1c40f',
    borderColor: '#f5c876',
  ...
  const camera = {
    ...
    targetFocusX: initialFocusX,
    targetFocusY: initialFocusY
  };
  ...
  const playerRenderData = [
    {
      id: 0,
      headbandColor: colors.player1Headband,
      controls: { up: 'KeyW', left: 'KeyA', right: 'KeyD' },
      isTagger: true,
      tagTimer: 0,
    ...
    {
      id: 1,
      headbandColor: colors.player2Headband,
      controls: { up: 'ArrowUp', left: 'ArrowLeft', right: 'ArrowRight' },
      isTagger: false,
      tagTimer: 0,
  ...
  platformData.forEach(data => {
    const platformBody = Bodies.rectangle(data.x, data.y, data.width, data.height, {
      ...platformOptions,
      angle: data.angle,
      label: data.label
    });
    platformBody.renderData = {
      width: data.width,
      height: data.height,
      colorBase: colors.platformBase,
      colorTop: colors.platformEdge,
  ...
  setupCollisionEvents({
    ...
    playerBodies,
    tagCooldownTime,
    groundCheckThreshold,
    jumpStrength,
    onTag: () => {
      flashOpacity = 0.3;
      console.log('Tag! Roles swapped. Flash activated.');
    }
  });
  ...
  function gameLoop(timestamp) {
    ...
    updateCamera(camera, canvasWidth, canvasHeight, worldWidth, worldHeight, zoomPadding, minZoom, maxZoom, zoomLerpFactor, cameraLerpFactor, playerBodies);
    ...
    playerBodies.forEach(pBody =>
      drawPlayer(ctx, pBody, dt, colors, { playerHeight, playerWidth, playerCornerRadius, legAnimationSpeed, tagCooldownTime })
    );

    ctx.restore();

    drawFlash(ctx, canvasWidth, canvasHeight, () => flashOpacity, op => {
      flashOpacity = op;
    });
